...
@app.route('/registro_editar/<string:idcontacto>')
def registro_editar(idcontacto):
    conn = pymysql.connect(host='localhost', user='root', passwd='', db='contacto')
    cursor = conn.cursor()
    cursor.execute('select idcontacto,nombre,correo,telefono,problema,direccion,fecha from contacto1 where idcontacto = %s', (idcontacto))
    dato = cursor.fetchall()
    return render_template("registro_editar.html", dat=dato[0])


@app.route('/registro_fedita/<string:idcontacto>', methods=['POST'])
def registro_fedita(idcontacto):
    if request.method == 'POST':
        nombre = request.form['nombre']
        correo = request.form['correo']
        telefono= request.form['telefono']
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='contacto')
        cursor = conn.cursor()
        cursor.execute('update contacto1 set nombre = %s, correo = %s, telefono = %s, problema = %s, direccion = %s, ''fecha = %s where idcontacto = %s', (nombre,correo,telefono,problema,direccion,fecha, idcontacto))
        conn.commit()
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='contacto')
        cursor = conn.cursor()
